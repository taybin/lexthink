defmodule Lexthink.AST do
  ...
  @spec table(binary) :: :term
  def table(name), do: table([], name, [])

  @spec table(:term | binary, binary | Keyword) :: :term
  def table(:term[type: :'DB'] = db, name), do: table(db, name, [])
  def table(name, options) when is_binary(name) do
    table([], name, options)
  end

  @spec table(:term | [], binary, Keyword) :: :term
  ...
  @private
  @spec table_option_term({atom, any}) :: :term_assocpair
